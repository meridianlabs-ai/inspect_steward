...
my-sweep/
  ...
  _steward.md         # authored  — your eval policy
...
Gitignored is **not** the same as disposable: `logs/` and `logs-archive/` are ignored by git because `.eval` files are large outputs shared through a log store rather than source control — they are still durable. Only `.steward/` is safe to remove, and even that only when nothing is running: a worker that has not yet reached `eval_set()` dies with its instructions, and deleting the claim file under a running tend lets a second tend run concurrently.
