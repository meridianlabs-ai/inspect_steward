---
title: Inspect Flow
llms-description: "Running an Inspect Flow spec under Steward: installation, what Steward overrides in a worker, and the two declarations that do not survive."
---
...
| `--log-dir <scratch>` | Flow writes artifacts and scans for prior logs before `eval_set()` is reached. Each worker gets its own scratch directory for that; the run's log directory is set at the `eval_set()` call. |
