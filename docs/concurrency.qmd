---
title: Concurrency
llms-description: "Matching sample concurrency to inference concurrency: how sandboxes, samples, tasks and model connections are bounded, how the automatic ramp finds the right level, worker processes, and Docker configuration."
---
...
You can change these defaults in your call to `eval_set()`:
...
If all checks pass, then tend adds an additional 20 samples to the limit. This ramp continues up to 150 as long as the checks continue to pass. The ceiling is where it stops: Steward never asks to go past the range it was given.
...
Widening it is yours to ask for, and narrowing it works on a live run: a task the ramp had taken to 150 comes back to a new ceiling of 100 at the next tend.
...
The cost is startup, since every worker imports your definition and its dependencies before it can run. For plain `eval_set()` definitions that is seconds; [Flow](flow.qmd) and [Hawk](hawk.qmd) can have longer startup times.
...
Each `steward tend` records the host's available memory and swap alongside the fleet's resident memory. `status.md` and `steward collect` show the current figures, headroom (available memory plus free swap), and a trend over the last two hours of tends:

```
memory: available 12.0 GiB of 64.0 GiB · swap 2.0 GiB of 8.0 GiB used · headroom 18.0 GiB (28%) · fleet 41.0 GiB resident
  headroom falling 1.3 GiB/h over 1h40m (11 points) · exhausted in ~2h10m
```

While headroom is under 10% of RAM the ramp does not climb. At that point, or when headroom is projected to run out within an hour, a `memory` item goes to the agent. On Linux the agent adds a swap file, which takes effect for running workers immediately. Where it can't (no `sudo`, containers, macOS) it lowers `max_samples` and gives you the commands to run. See [Memory](runbook.qmd#memory) in the runbook.
