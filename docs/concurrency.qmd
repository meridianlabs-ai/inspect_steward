...
That holds for all of them, with no exceptions to remember: `max_workers`, `stall_after`, `samples_ramp`, `tend_interval`, and `policies`. All three spellings go through one parser, so a value that is refused in the file is refused the same way and with the same message everywhere else — `STEWARD_MAX_WORKERS=yes` fails naming the `True` that YAML made of it, exactly as `max_workers: yes` does.
...
Inspect's knobs are not in this table, and that is the ownership rule rather than an omission: `max_tasks` and `max_samples` belong to your definition, and Steward has no spelling of its own for either.
