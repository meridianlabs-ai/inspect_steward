...
It belongs in your definition — `eval_set(max_tasks=...)` — and `_steward.md` refuses it by name, on the same rule that refuses `max_samples`: **inspect's words go in the definition; `_steward.md` holds only words `eval_set()` does not know.** Steward reads the value your definition asked for and applies it to the fleet, so the number means what it says even though each worker is handed only its own share of it.
...
Sample concurrency is a property of the eval — its author knows the workload — so `max_samples` belongs in your definition and `_steward.md` refuses it by name. An explicit value **pins** the setpoint: Steward honors it as written and never moves it.
...
- **The envelope.** The range itself, settable in `_steward.md`:
...
Standing values belong in `_steward.md` front matter (`max_workers`, `samples_ramp`, `stall_after`, `tend_interval`) or your definition (`max_tasks`, `max_samples`); the CLI is for the run where conditions differ.

**Two of the three lapse; `--max-samples` does not.** Fleet width and process packing are recomputed from scratch every turn, so a flag that is not repeated simply stops applying. Sample concurrency is different, because a value there does not just set a number — it pins the setpoint and switches the ramp off. So the pin is remembered for the rest of the run, and the way to release it is to write a range in `_steward.md`:
