...
- **`max_sandboxes` is a machine-wide budget.** A task's containers never exceed its running samples, so Steward bounds the fleet-wide sum of sample setpoints instead of the containers: workers start at a share of the budget rather than at the ramp's floor, a fleet found above it is brought back to that share, and no step is admitted that would take it past. That share is free to be smaller than the ramp's floor — the floor is where discovery starts, not a promise your host can afford it. The budget is your definition's `max_sandboxes` if it sets one, otherwise the sandbox provider's own default; elastic providers (k8s) cap nothing. A `max_samples` you pinned yourself is never lowered — if ten pinned tasks ask for more than your `max_sandboxes` allows, Steward reports the contradiction rather than picking which of your two numbers to believe.
...
If your tasks use Docker sandboxes, there is a ceiling below all of the above that has nothing to do with your machine's size, and it is worth knowing about before it bites.

Every sandboxed sample gets its own Docker compose project, and every compose project gets its own bridge network. Docker allocates those out of `default-address-pools`, and the built-in value carves two ranges coarsely — sixteen networks each, so **about thirty across the whole host**. It does not scale with your processors. On a 64-core box the Docker provider will happily offer 128 concurrent sandboxes and Docker itself will stop at thirty, failing every sample after that with
...
which reads like a network fault rather than a limit somebody is allowed to raise.

`steward launch` checks for this and tells you — once per workspace, and only when the run would actually want more sandboxes than your daemon can give it, so a laptop where thirty is ample hears nothing. It will offer to write the fix into your `daemon.json`, keeping a backup and leaving whatever else is in the file alone:
...
The same two ranges Docker already claims, carved finer: 512 networks instead of 32. Declining is the default and prints the same JSON with the file to put it in.

**Steward never restarts the daemon**, and nothing takes effect until you do — a restart stops every running container on the machine, which on a shared box is somebody else's work, so it is yours to time. The launch prints the command for your platform.

Background: [Docker's default address pools](https://straz.to/2021-09-08-docker-address-pools/).
...
One of inspect's words is Steward's here: `notification`. See [Getting told](workflow.qmd#getting-told) — the channel Steward posts to and the channel your workers post from have to be one channel, so Steward owns the setting in all three spellings and both halves are wired from it.
...
**The other is `log_dir`.** Steward decides where a run's logs go — the fleet is watched from that directory — so there is no `--log-dir`, no `STEWARD_LOG_DIR`, and `INSPECT_LOG_DIR` is refused at launch rather than quietly ignored. What Steward will not do is *override* a `log_dir` your definition names; where your definition names none, see [Where your logs go](workflow.qmd#where-your-logs-go), which is the case worth preferring.
