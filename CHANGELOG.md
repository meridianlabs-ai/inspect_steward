...
- Configuration reading: `read_eval_set()` enumerates the tasks of an eval set definition — a Python file culminating in `eval_set()`, or an Inspect Flow spec (Python or YAML, via the `[flow]` extra) — returning a `Manifest` with per-task identity, display keys, sample counts, and epochs. Requires an `inspect-ai` release with eval-set capture support.
