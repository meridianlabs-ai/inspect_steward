...
While Steward starts with `max_samples=50`, it will also attempt to automatically ramp sample concurrency according to how well the model provider is handling the load.
...
Note that `max_sandboxes` is a machine-wide budget that bounds the fleet-wide sum of sample setpoints, where workers start at a share of the budget instead of at 50. The budget is your definition’s `max_sandboxes` if it sets one, otherwise what the sandbox provider reports (e.g. Docker’s 2 \* available CPUs). Elastic providers such as k8s have no limit.
