...
| [plan.md](plan.md) | **The order of construction.** Thirty-four independently designable and testable steps, each citing the sections its design pass starts from, with the milestone gates located precisely and Hawk's in-pod stage after ship. | Sequence and gates settled; step internals are per-step work |
...
| [scheduling.md](scheduling.md) | What `reconcile` actually **decides**: how many tasks run at once and how many processes they are poured into, spawning task-major, how the three concurrency knobs are set, when scan passes run, and why a failed task is adjudicated rather than retried. | Settled |
...
- **The definition is the single source of truth** for what an eval set is. `_steward.md` carries Steward's own settings — fan-out width, tend interval, notification policy — and is structurally barred from naming anything the definition owns: those keys are refused by name, with a message saying where they belong.
...
- **A timer guarantees the mechanical tend; the agent supplies judgement.** No long-lived supervisor — `reconcile` is a pure function and the claim is held for the seconds a tend takes, so a timer, an agent, or both can call it. That split is what makes an absent agent survivable: the fleet keeps converging and only decisions accumulate. The agent tunes, groups, investigates, and writes ([agent.md](agent.md)), and may be attached, periodic, or merely transient. **One condition is exempt** — a worker parked on a human approval holds its slot until a person answers, which neither the timer nor the agent may do, so a definition with human approvers loses throughput as well as accumulating decisions ([execution.md](execution.md), *The parked worker*).
...
- **One automatic tier, then adjudication.** A sample gets the `retry_on_error` attempts its definition asked for; every further attempt, at sample or task level, is a ruling. `_steward.md` may grant that ruling in advance, but nothing is pre-authorized by default, and Steward has **no** automatic response to an error class.
- **Mechanics ship with the package (`steward runbook`); standing rules live with the project (`_steward.md`).** One file, two halves: YAML front matter Steward executes unattended, prose an agent interprets when it arrives. Steward proposes changes to it and never writes it.
...
execution.md's *Changes required in inspect_ai* is the single list of what Steward needs from Inspect. Landed: capture, selection, error-handling overrides, operational overrides, and `max_samples` in the capture manifest's options. Outstanding: early pruning, public directory operations, notification-outside-an-eval, the dataset `limit` override a smoke run needs, a `max_sandboxes` override, a capture manifest that records each task's sandbox type and epochs reducer, and the pair that gives a detached worker a path to a human — ACP on in worker mode, with the parked state readable over the control channel. [roadmap.md](roadmap.md) orders these by when they bite — **only notification and that pair are on the critical path, and notification has a workaround.**
