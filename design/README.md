...
- **The definition is the single source of truth** for what an eval set is. `_steward.md` carries Steward's own settings — worker ceiling, tend interval, notification policy — and is structurally barred from naming anything the definition owns: those keys are refused by name, with a message saying where they belong.
