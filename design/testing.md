...
**One flake is closed, and its cause was not where the design predicted.** Two tests in the Flow path — `tests/evalset/test_selection.py::test_identifier_correlates_by_definition_type[flow_spec.py]` and `tests/worker/test_flow_worker.py::test_a_flow_fan_out_leaves_the_log_directory_to_the_logs` — failed intermittently under parallel load at roughly one run in three, and passed alone. The obvious suspect was Flow's unsynchronized pre-boundary writes ([execution.md](execution.md) §13 q1), which is a hazard that document predicts. It was not that. The failing worker died on `ModuleNotFoundError: No module named 'inspect_ai.model._providers.none'` — a module that exists — because **the hawk tests reinstall inspect_ai into the interpreter running the suite.** `hawk local eval-set --direct` calls `uv pip install` into the current environment, which uninstalls this project's editable inspect-ai and rewrites the package tree from hawk's own git pin; a sibling xdist worker importing during that window finds a half-written package. Flow was the visible victim only because `get_model("none")` is a *lazy* import late in its startup, so it has the widest exposure.

Two lessons, and the second is the larger one. **A test that mutates the interpreter cannot run beside the others**, so `network` is deselected by default rather than merely marked (`pyproject.toml`). And **it was corrupting results even when it passed**: the venv is left pointing at hawk's snapshot afterwards, so every later run tests a different inspect_ai than the checkout — which happened, silently, for a whole working session before anyone noticed a park behaving like an older build. `uv sync` restores it, and `python -c "import inspect_ai; print(inspect_ai.__file__)"` is how you check.
