...
**`max_samples` is the false alarm.** It bounds concurrency *within a task*, not within a process, so single-process Hawk was already multiplying it by however many tasks ran at once — up to `max_tasks`, which is also 1000. Worker mode swaps that multiplier for the worker count, which Steward caps at ten by default ([scheduling.md](scheduling.md), *Launch everything, up to a ceiling*). Fifty concurrent tasks in one process is 50,000 potential samples; ten workers is 10,000. The knob that looked like the hazard is the one worker mode makes *smaller*.

**And Hawk's value is a pin, so the ramp never sees a Hawk run.** `InfraConfig.max_samples` is `1000` rather than `None`, and `run_eval_set` passes it to `eval_set()` unconditionally — so capture records it in `options` and [scheduling.md](scheduling.md) §3.2's precedence reads it as the definition expressing a setpoint. Every Hawk task therefore launches pinned at Hawk's number, `samples_ramp` has nothing to govern, and none of §3.5's discovery applies. Worth stating flatly because the general documentation promises a ramp, and a Hawk user is the one reader for whom that promise is never kept. Verified rather than inferred: capturing a Hawk config that sets nothing yields `max_samples: 1000, max_sandboxes: 20, max_tasks: 1000`, where the equivalent Flow spec yields `max_samples: None`.

**This is Hawk's policy and Steward keeps it**, which is the same rule applied to a frontend that the rule was written for an author: sample concurrency is a property of the eval, whoever states it. Steward cannot distinguish a considered 1000 from a sentinel anyway — capture records the effective value and no provenance — and the alternatives are worse than the cost. Reading Hawk's YAML to find out whether the number was written by a person is the config archaeology driving Hawk's CLI exists to avoid; a threshold above which a pin stops counting is a heuristic that would eventually override somebody who meant it. The cost is accepted and narrow: a Hawk user who wants discovery sets a smaller number in the infra config, where the rest of their concurrency lives.
...
The narrower claim replaces the old blanket one: it is not that every knob must be bounded by Steward, but that **the ones with no feedback signal must be**. Connections coordinate through rate limits; samples were never the multiplier they appeared to be; sandboxes have nothing, so Steward caps their sum. What neither Steward nor an agent may do is exceed the infra config's sandbox number, because unlike wall clock, exceeding this one kills the pod.

Note what the pin above costs here specifically: with `max_samples` fixed by the config, the sandbox sum-cap has nothing left to gate. All three of its enforcement points — the spawn clamp, the over-budget cut, and step admission — turn on a ramp being in force, because under a pin the setpoint is a number the config's author wrote rather than a starting point Steward picked. A Hawk run has no ramp, so a config pinning a high `max_samples` across many tasks can ask the host for more than its own `max_sandboxes` allows, and the process-global sandbox limiter is what actually holds the line, exactly as it did before worker mode. That is the general case scheduling.md §3.6 already names: two numbers the same party owns, contradicting each other, reported rather than resolved.
