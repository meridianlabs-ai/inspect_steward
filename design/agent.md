...
The score column is live-only in the same sense while a task runs. Inspect writes a log's results at completion, so the final figure exists only once the task has finished; until then the column carries an **interim** figure from the control channel's completed-only scoring pass — the task's own metrics and reducers over the samples scored so far, holding nothing and calling no grader. It is read by the same declaration the final figure will be read by, so the running row and the finished row agree on *which* number it is. A turn asks only when the worker's sample listing shows more scored samples than the last harvest described, keeping the previous figures otherwise, because the same count is the same figures. The final figure replaces the interim one when the log lands. Nothing marks the difference: the row's own counts say how much of the task the figure is over.
...
- **One exception, and it is Steward's rather than the agent's: the smoke.** A rehearsal's own logs are read whole, because whether a model resolved to a real context window and whether reasoning is being replayed are answerable nowhere else. It stays an exception rather than becoming a precedent because four bounds hold together — only `.steward/smoke/`, bounded by the rehearsal's slice, paid once at launch rather than every tend, and the transcript never leaves Steward. What reaches an agent is the digest's verdict, which is the discipline above rather than a departure from it: the evidence stays where it was read and the answer is what travels.
