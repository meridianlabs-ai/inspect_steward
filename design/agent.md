...
- **Write `_steward.yaml`.** Steward proposes; the human writes.
...
**Only with a ruling** — every re-run past the automatic tier, at sample or task level ([execution.md](execution.md), *Two tiers, not three*). `_steward.yaml` may grant that ruling in advance, in which case acting on it is executing a decision already made.
