...
| `max_workers` | `_steward.yaml`'s | how many worker **processes** those are divided into | unbounded — a process per task |

The ownership column is not decoration: `max_tasks` is a word `eval_set()` knows, so the definition says it and `_steward.yaml` refuses it, while fanning a run across processes is Steward's invention and has no other home ([workflow.md](workflow.md) §5.9). An unset `max_tasks` means *everything at once* rather than `eval()`'s one-at-a-time, which is the one place the fleet reads inspect's word differently and is stated in §3.1.

**Ownership decides the file, not the run.** One run may still say `max_tasks` without editing the definition — as `steward launch --max-tasks`, or as `STEWARD_MAX_TASKS` or `INSPECT_EVAL_MAX_TASKS` in the shell — because a Steward run *is* an `inspect eval-set` invocation and reads the environment that CLI documents ([workflow.md](workflow.md) §5.9, *Inspect's words*). What the rule forbids is the committed key, where a contradiction would ossify. The two surfaces differ in another way that follows from the same place: inspect's words are settable at `launch` alone, because a run's shape is decided when it is launched, where Steward's own are settable on every command that acts.
...
| | |
|---|---|
| the **shell** this turn ran in — `STEWARD_MAX_SAMPLES`, or inspect's own `INSPECT_EVAL_MAX_SAMPLES` | wins outright — somebody typed it, now |
| the **run**, as `steward launch` resolved it into the committed manifest | next: typed for this run, and durable because no shell survives to the 02:00 tend |
...
The definition's row is the one that needed something from upstream, and it now has it: `eval_set()`'s capture records `max_samples` in the manifest's `options`, so a definition asking for 60 gets 60 instead of being silently replaced by Steward's 40. Before that field existed there was no read-side workaround either — a landed log records only the *effective* value, which is whatever Steward imposed.

**The run's row needed the other half of the same upstream change.** `options` records what the definition asked for — a runner already knows what it set and cannot otherwise learn what it displaced — so the run's own value lives in the manifest's `overrides` beside it, honoured by the capture that wrote it. That is what keeps the enumeration and the fleet in agreement: an `epochs` or `limit` override changes a task's sample count, and a manifest counting one run while the workers ran another would make every convergence check wrong without failing anything.

**"The definition" includes whatever frontend produced it**, which is not a quibble: Hawk's infra config sets `max_samples` unconditionally at a default of 1000, so every Hawk run occupies that row and is pinned, and none of §3.5 applies to it ([hawk.md](hawk.md)). Capture records the effective value and no provenance, so a framework default and an author's considered number are the same reading here — deliberately, because the alternatives are parsing each frontend's config to see who typed what, or a threshold above which a pin stops counting. Sample concurrency is a property of the eval whoever states it, and a frontend stating it is a policy that frontend has chosen.
...
The objection was never that the arithmetic needed judgement; it was that an autonomous ramp would be **a policy shipped in the binary that nobody chose**. That is answered by making the envelope the choice: the ramp acts only inside a range someone can see and set (`samples_ramp` in `_steward.yaml`, defaulted to 40–200 and disabled by any explicit `max_samples` anywhere), which is workflow.md's *authorize at 10pm, do not interrogate at 3am* made standing rather than conversational. And the accepted cost stopped being acceptable on its own terms: undershoot compounds (workflow.md §10.6), the hours Steward exists for are exactly the unattended ones, and a run left at a third of its viable concurrency all night is not a conservative outcome — it is the expensive one.
...
**The budget binds in three places, because refusing to climb is only one of them.** An earlier version of this section stopped at the admission rule above, and that left the failure it was written for untouched: a fleet does not *climb* into the multiplication, it **starts** there. Every worker spawns at the ramp's floor, the floor knows nothing about the host, and ten tasks at 40 against a budget of 28 is the same 10× overshoot the table names — reached before the ramp is asked anything. The admission rule then declines every step and reports the budget as binding, which is an accurate account of a machine already over-committed and does nothing about it. So:
>
> - **at spawn**, `reconcile` clamps a worker's starting setpoint to an even share of the budget;
> - **at every tend**, a fleet whose setpoints already total more than the budget is cut back to that share — gated like the narrowed-range reduction and not like a step: no window, no spacing, no hold, because a bound already exceeded is not capacity being declined;
> - **at every step**, the admission rule above, which is what keeps it there.

**The share is free to land below the ramp's floor, and has to be.** A budget of 28 across ten tasks does not divide into a floor of 40, so a cut clamped at the floor would leave the host exactly as over-committed. The floor is where *discovery* starts, not a promise the machine can afford it — and the ramp climbs back out through the gates as tasks finish and the sum falls. This is the one move in the loop that goes under the floor.

**Spawn needs the number a tend earns, so the tend writes it down.** The budget is readable from a running worker and the spawn decision is taken before that turn's live read, so an undeclared budget is unknown on the first tend and recorded in the observation for the next one. A definition that declares `max_sandboxes` needs none of that — capture put it in `options`, and it is known at launch. The exposure that remains is one tend of a first spawn under an undeclared budget, which is the price of the number living in the provider.
...
**What the sum-cap deliberately does not do is lower a pinned setpoint** — not at the spawn clamp either, which is checked against the ramp being in force at all. A definition that pins `max_samples=200` on ten Docker tasks can still ask the host for more than the provider says it supports, and those are ten numbers a person chose against a budget the same person owns: the class of contradiction Steward reports rather than resolves. The distinction the clamp turns on is `resolve_max_samples`'s own — under a pin the level is a *setpoint* somebody wrote, under a ramp it is a *starting point* Steward picked, and Steward may correct its own starting point without asking.
...
One wrinkle flagged here and fixed upstream: Docker's `default_concurrency()` called `os.cpu_count()`, which reports the host's processors rather than a container's cgroup quota. A pod limited to 2 CPUs on a 64-core node reported 64, so the budget Steward reads back off the workers was 32× the truth in precisely the deployment where over-allocating kills the pod ([hawk.md](hawk.md)). **[inspect_ai#5164](https://github.com/UKGovernmentBEIS/inspect_ai/pull/5164) reads the real number** — the CPU quota from the cgroup filesystem and the CPU set from the affinity mask — for `max_sandboxes`, the docker CLI limiter, and `max_subprocesses` alike. The inflation happened inside the provider, so it was Inspect's to fix, and the correction arrives in the number Steward already reads. (Hawk's `memory_monitor` was the cited precedent and turned out not to be one: it polls the cgroup for memory and for CPU *throttling* counters, and never reads the quota.) In a Hawk pod it is moot regardless, because the infra config sets `max_sandboxes` explicitly and the default is never consulted.
...
**Built as `scanerror:{scanner}:{type}@{frame}`, and the key comes from the traceback alone.** Scout writes three error columns and only one of them classes: `scan_error` is `str(ex)` where the shared parser wants `repr(ex)`; `scan_error_type` is the literal string `"refusal"` on every error row regardless of what threw; `scan_error_traceback` is a real `traceback.format_exc()` and reads exactly as a sample's does. It is a full anomaly window — same investigation, same rulings, same signoff gate — with one row of the honesty matrix of its own: **`rerun` is refused**, along with the three sample marks. The samples are fine and only the reading of them failed, there is nothing to requeue, and the retry a re-scan would need is scout's resume rather than a respawn, which Steward has no verb for. `accept` and `dismiss` are the answers.
...
**Coverage is recorded rows against landed samples, and the resume is where it gets hard.** The numerator is free — every scan row already names the log it came from and the transcript it is about, and the tend already holds the map from log basename to task, so it accumulates inside a pass that was already happening. A transcript counts as recorded once **every** scanner has answered for it, which is upstream's own resume predicate rather than a rule invented here: a sample two of three scanners reached is one the third will be sent back for. An *errored* row still counts, because that failure is already reported as its own class above and counting it twice would put one problem in two places that disagree about what to do with it.

The denominator is the log header's `total_samples` — exact, and exactly wrong for one shape of task. After a retry the new log carries the samples that already succeeded, keeping their uuids, while their scan rows still name the **superseded** file. Counting rows per log undercounts by the whole reused population; counting them across logs hides the re-runs nothing has scanned yet behind samples that have left the results. The only identity that survives a retry is the sample uuid, and no header carries it — so a task that was resumed *and* has scan rows pays one sample-summaries read, and its numerator is intersected against what its current log actually holds. Every other task pays nothing.

**And that read can fail, which is a third state rather than a fallback.** The two consumers of the uuid set owe a failed read opposite answers. For the narrowing, *absent* is right — fall back to the location test, which loses nothing that was not already lost. For coverage it is the one thing that must not be read as absent, because absent there means *this task was never resumed, so counting the rows is exact*, and for a resumed task the rows are the union across attempts: four rows against four landed samples reads as fully scanned when two of those rows are about samples the retry replaced. So the task is marked **unknown**, held out of the run's totals so the gap stays a number somebody counted, and rendered as `?/4` on the table, in the note under it, and in `analysis.md`. *Nothing is known to be scanned* and *nothing was scanned* send a reader after two different problems, and the surfaces are not allowed to conflate them.

**The same read fixes a defect one layer over.** Every report-facing count narrows the census to *what is in the results*, and the test was *the instance's log is the task's current attempt* — which silently drops a scan finding on a reused sample, taking with it the ruling meant to cover it. For scan-shaped instances the test is now *is this sample in the current log*, keyed on the uuid set coverage has already read, so the count and the list it summarizes come from one read and cannot drift apart.

**A scanner that produced no file at all covers nothing, and that is the whole point of the intersection.** It is the one case where the cheap reading is actively dangerous: leaving a scanner out of the intersection because it has no parquet lets the scanners that *did* run report the run as fully covered on its behalf, which is exactly the two situations coverage exists to show — a scanner added at a re-launch that will never revisit the transcripts already landed, and one that never started. So a configured scanner with no file contributes an *empty* answer, and so does one whose file will not read, on the stronger version of the same reasoning: what that file said is unknown, and unknown cannot be counted as answered.

**Coverage is reported and deliberately does not refuse.** Signoff reads the errored half — a `scanerror:` window is refused like any other open window — and leaves the unreached half to the column and the note. The reason is the one that retired the acknowledgeable `scan_incomplete` item: what closes a coverage gap is scout's resume over the scan directory, which Steward has no verb for, so a blocker over a scanner added at a re-launch would wedge the run permanently and the only escape would be an ack — a wave-past wearing a decision's clothes. The person signing is told the number instead.
