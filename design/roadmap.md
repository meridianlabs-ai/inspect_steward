...
**A workspace can state its own standing rules.** `_steward.yaml` replaces `policy.md` — settings Steward acts on unattended beside a `policies` key an agent interprets — and the rule that keeps it safe is enforced rather than documented: a key the definition could express is refused by name, and anything unrecognised is rejected outright (step 12).
...
8. **`signoff`**, `anomalies.md`, the gate latch, and curation into `logs-archive/`. **Built** ([plan.md](plan.md) step 26), and it carries one piece the design record did not name: an accepted log is *amended* to say `success`, so the artifacts on disk agree with the decision a person made about them, and a Steward-side latch stops reconcile respawning what they accepted.
...
Scanning is the largest piece and the most valuable — three steps in [plan.md](plan.md), because the fleet contract, the tend lifecycle, and turning results into decisions have different dependencies and different failure modes: scans riding the workers online with the runner owning the scan directory's bracket, the sync/coverage/finalize lifecycle in the tend, a flagged sample becoming an anomaly the run cannot be signed around, and `analysis.md` mirrored into the log directory. The smoke gate and store publication are small by comparison. Note archiving is **not** here — it moved into M3 with signoff, since curation is part of the attestation rather than a later tidy-up.
...
| 9, 10 — `max_sandboxes` override and sandbox type in the manifest | **closed** | both dissolved rather than landing. 9 is in at schema version 3 and deliberately unwritten: it was to carry a per-worker share of a divided budget, and there is no division. 10's `options["max_sandboxes"]` half landed; the per-task sandbox *type* half is unneeded, because the live config read says which tasks are host-bound directly — an elastic task reports no limiter. `workers × 2 × cores` is retired by the sum-cap's three enforcement points ([plan.md](plan.md) step 27), and the provider's own count is corrected by [inspect_ai#5164](https://github.com/UKGovernmentBEIS/inspect_ai/pull/5164) |
| 6 — public directory operations (incl. `archive_dir`) | **closed** | Signoff curates into `logs-archive/` and needed no reimplementation after all: `ObservedLogs.current` and `_current` already answer *which attempt is the result*, so curation consumes `TaskObservation.superseded` and adds no predicate that could drift from `eval_set()`'s. Its second half — a batched header read that degrades instead of raising — Steward has likewise already reimplemented, and would keep doing so |
