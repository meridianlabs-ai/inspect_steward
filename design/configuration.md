...
| *(operational overrides)* | Carried in the selection document's `overrides` container rather than a channel of their own: `log_dir` and `max_samples` today, with a dataset `limit` and `max_sandboxes` joining them at schema version 3 ([execution.md](execution.md), items 8 and 9). None changes what an eval means, and none participates in task identity. The options Steward overrides *semantically* are not here at all — `fail_on_error` and task retry are applied by selection mode itself, and so is `acp_server` (item 12). |
