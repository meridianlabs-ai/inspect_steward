...
3. **Overrides whitelist.** Exactly which `eval_set()` kwargs Steward may override in workers (`log_dir`, `display`, `log_level`, `retry_attempts`, `ctl_server`, `max_tasks`, ...) and what happens on conflict with definition-specified values. *Partly settled:* the whitelist is now a model rather than a list — the selection document's `overrides` container — and it stays purely operational: an override may change how a worker is *operated*, never what is evaluated. The semantic ones (`fail_on_error=False`, task retry off, `acp_server` on) are applied by selection mode itself, which keeps this channel from needing a second, riskier tier.
   ...
   **Two of its fields are Steward's word rather than a per-run override, and the set is named** (`_workspace.directives.STEWARDS`). `log_dir` and `notification` both answer a question about the *deployment* rather than about the eval, so neither gets the generated `STEWARD_*` alias and `--flag` every other field gets; each has a Steward setting carrying all three spellings instead. `notification` is still *written* into every worker's overrides — as `True`, whenever a channel resolved — because a worker's `eval_set()` only builds an Apprise instance when its own argument is truthy ([workflow.md](workflow.md) §11). Exporting the URL alone leaves the fleet silent while Steward posts normally, which is the failure the carve-out exists to prevent.
