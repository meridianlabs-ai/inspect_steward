...
4. Operational overrides: carried by the selection document's `overrides` container (`log_dir`, `max_samples`, `limit`, `max_sandboxes`) rather than a separate channel. An environment variable could not serve — `INSPECT_LOG_DIR` and its siblings are *defaults*, and `eval_set()` declares `log_dir` with no default, so a definition's explicit argument always wins. The error-handling options this once had to carry (`fail_on_error`, `continue_on_fail`) are hard-coded by selection mode instead, and `retry_on_error` stays with the definition. *Landed*, the container and all four fields, at schema version 3. `max_samples` is concurrency and `limit` is a slice; neither substitutes for the other, and the smoke needs the second one.
