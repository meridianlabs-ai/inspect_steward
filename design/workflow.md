...
| a `log_root` is configured (`--log-root`, `_steward.yaml`, `STEWARD_LOG_ROOT`, or inspect's own `INSPECT_LOG_DIR`) | `<root>/<workspace directory name>` |
...
**The second rung is a default and never an override, which is what admits it.** §5.9's *Inspect's words* rule says Steward refuses every way of *moving* a `log_dir` the definition stated — no flag, no alias. A root does not move one: a stated `log_dir` is returned untouched, unrebased and unprefixed. What it answers is silence, which is not an assertion. And what it expresses — *where this machine keeps eval logs* — is something no definition can portably say, since a definition knows neither the root nor the workspace's name.

**`INSPECT_LOG_DIR` is the weakest spelling of this second rung, not a refused override.** A root can be named on the command line (`--log-root`), in `_steward.yaml`, in `STEWARD_LOG_ROOT`, and — below all of those — in inspect's own `INSPECT_LOG_DIR`, which sits last because it is the broadest word for the same thing. That it is inspect's is exactly what admits it: a root *is* a default in inspect's own sense (`eval()` falls back to `INSPECT_LOG_DIR`, and an explicit `log_dir=` displaces it), so reading it as the lowest-precedence root contradicts none of the *Inspect's words* rule — it never moves a stated `log_dir`, and it becomes `<root>/<workspace name>` rather than the directory a fleet would write straight into. The earlier design refused it at launch on the fear it would do the latter; forcing the resolved directory on every worker through the selection is what makes the fear unfounded. An explicit decline (`--no-log-root`, `log_root: false`) still beats it, because a decline is an assertion and a default never displaces one.
...
**And it is reported, which it did not need to be before.** `status.md` and `steward status --json` name the directory, because the thing that made it guessable stopped being true: an agent is told to reach for `samples_df` and `read_eval_log_sample_summaries`, and under a root the workspace has no `logs/` at all — so a guess finds nothing rather than failing. The alternative was pointing the runbook at `.steward/manifest.json`, which would put a private file in the agent contract and place it in the one directory this design documents as safe to delete. Not in the terminal renderings, where the reader is standing in the workspace and the compact output is for what changed.
...
| `steward notify [--kind attention\|stopped]` | agent | Send the human a message that carries judgement, through the run's notification channel. The other four kinds are Steward's alone — see *Six kinds*. |
| `steward signoff --by NAME [--note TEXT] [--again]` | a person, or an agent relaying one | Attest that the results are accepted, and end the run. Runs a final turn under the claim, refuses with every blocker at once while anything is unnamed, curates superseded attempts into `logs-archive/`, records who, when, the manifest digest and the exceptions accepted, and disarms the timer. `--publish` puts the signed logs into the configured reuse store, and is the only thing that ever does — there is no setting that publishes without being asked. |
...
Two things the table is meant to make obvious. **Almost everything is agent-facing**: the human's own surface is `init`, `signoff`, and asking the agent questions in prose. And **`signoff` is the one command whose *decision* is never the agent's** — an agent that notices a run is ready and tells the person is doing its job, and carrying out their answer is the same act as relaying a ruling through `rule --by`. What is recorded is the name of whoever decided, so a signature stays traceable to a person whichever process typed it; what would make the record meaningless is not an agent at the keyboard but a signature nobody asked for. The runbook says so plainly.
...
Anomaly state — what is open, what was proposed, what was ruled and how — is a **pure fold over the journal**, not a separately maintained file. Any `anomalies.json` would be a cache of that fold, living in `.steward/` with the rest of the disposable state — and none is written today: the fold replays in the time a turn already spends, and a cache waits for a measurement that demands it. (What `.steward/` does hold is `classed.json`, which caches *classification reads* rather than fold state.) The property this buys is the same one that made reconcile worth writing as a pure function: crash recovery for adjudication state is the normal code path, exercised on every tend rather than in a rescue routine nobody tests.
...
| `instance` | new instances joining a class, **batched — at most one event per class per turn**: a count, per-instance dedupe refs, and capped evidence (a few sample ids, the tasks and logs involved, one verbatim message). The authoritative instance set re-derives from the log directory every turn; the journal carries the time series and the decision trail | every tend, as observed; a deciding verb, for its targets |
| `opened` | a class has a window absorbing instances — its computed key, kind, and whether it looks like substrate | every tend, as observed; a deciding verb, for its targets |
...
| `action` | what was actually done — requeue, respawn, archive, pause, `ruling_applied` (a rerun ruling carried out, or an acceptance: warm requeues, landed invalidations, logs marked complete, convergences), `curated` (every superseded attempt one signoff moved, counted in one event) — and its outcome | Steward |
...
| `signoff` | terminal; who, when, the manifest digest, the exceptions accepted by name, and how many superseded attempts it curated | `steward signoff` — the human's decision, whoever types it |

**Every one of them is written today** ([plan.md](plan.md) steps 12–19 and 23): `initialized`, and — from every executed tend — `observation`, carrying task-state counts, running/spawning/queued/stalled/orphan counts, whether the definition has drifted from the manifest, the verdict, this turn's open item ids, and the concurrency settings in force. Two of those fields are not reporting material. The settings are what a turn reads back when `_steward.yaml` cannot be parsed, which is how degrading to the last known good is possible at all; the item ids are what the *next* turn diffs against, which is how a notification fires on a change rather than on a state. `action` is written for an archive, with the log's origin, its destination, and its reason. `acknowledged` is written by `steward ack` and by nothing else — a tend observes, and disposing of an observation is somebody's decision rather than a turn's. `paused`/`resumed` and `armed`/`disarmed` are likewise never written by a tend, and both fold as **switches** rather than accumulations: the last word wins, so a double pause or a resume with nothing to resume is the state it leaves behind rather than an error anybody has to handle. `raised` is written by `steward raise` and `collected` by `steward collect`, both of them agent verbs and neither of them a tend — `raised` folds per item id like `acknowledged`, and `collected` folds as a switch like the pause, so a `--since` that deliberately reaches backwards and then advances leaves the newer position behind. The anomaly six landed with step 23, split by who observes and who decides: a tend writes the mechanical three (`opened`, `instance`, `resolution`), the agent verbs write `investigating` and `proposal`, and `steward rule` writes `ruling` — with one deliberate crossover: a deciding verb first persists its targets' pending `opened`/`instance` events, because a `status` folds newly detected windows only in memory and a decision recorded against a window the journal does not hold would be skipped by the next fold. Replaying those events is safe by construction — the fold ignores instances whose refs it has absorbed. `signoff` landed with step 26, written by its own verb and by nothing else, and it folds as neither a switch nor an accumulation: **there is no un-sign event**, because a signature is a thing that happened — a second one amends the first and both stay in the record, so the fold's last word wins while whether the standing one still *stands* is computed against the manifest digest and the windows opened since it.
...
| notification channel | ~~`INSPECT_EVAL_NOTIFICATION` — reference-only *by design*~~ → **`notification` in the file**, the one spelling a 02:00 tend still has (see below) |
...
**Notification splits across the line rather than sitting on one side of it, and the split moved.** The table above used to send the *channel* to `INSPECT_EVAL_NOTIFICATION` on the grounds that a URL is a credential and files, shell history, and process listings are the wrong home for one. That argument was sound and it lost to a sharper one: a scheduled tend inherits almost no environment, so a channel that lives only in a variable is a channel the 02:00 turn does not have — which is the exact failure notification exists to prevent, arriving through the door notification cannot watch. `notification` is therefore a Steward key in all three spellings, and the discipline it departs from is preserved in the half that still applies: the value never prints and never propagates ([§11](#11-notification-is-the-gate-on-autonomy)). Notification **policy** is a different object again: which kinds go out, when to stay quiet. Inspect has no policy layer at all (*What Inspect already provides*), so there is nothing to contradict and nowhere else for it to live.
...
Putting it under `.steward/` also means the cleanup question answers itself. It is disposable state by construction, so it needs no special rule: each smoke clears the previous one, a failed smoke's logs stay put for as long as anyone wants to read them, and everything goes when `.steward/` goes. The one qualification is that the clearing must not fail *quietly*: everything downstream reads this directory as what this rehearsal produced, so a surviving log can satisfy the watch before a worker has written anything and land in the digest as this run's evidence. A rehearsal reporting somebody else's results is worse than one that refused to start. `inspect view --log-dir .steward/smoke` works on it like any other directory.
...
**It says nothing about tokens, and this paragraph used to say the opposite.** The earlier claim was that tokens per sample across the smoke extrapolate to the real run, which is a far better answer to "how big is this" than counting tasks and arrives exactly when someone is deciding whether to commit. It shipped, was read, and was wrong on its own terms: the paragraph immediately refused to project a *rate* on the grounds that two samples per task on a small pool says nothing about fleet throughput — and the same two samples say no more about spend. Worse, they are the front of each unshuffled dataset rather than a draw from it, so the extrapolation is not even a bad estimate; it is a confident-looking number computed over the samples an author happened to put first. A rehearsal is *bounded*, which is what makes it cheap, and inference from a bounded rehearsal to a whole run is the one thing boundedness does not support. What survives is the count: the digest says how many samples the run will produce, because the untruncated capture already knows that and nothing has to be inferred to say it.

The removed version also had a smaller correction inside it. An earlier draft said *cost*; Inspect reports dollars only where the definition supplied prices, and Steward does not supply them (see *Spend is not Steward's to manage*). Both retractions run the same way — the digest reports what it measured and stops.

**A smoke is valid for a manifest, not for a directory.** Whether a smoke still applies is answered by the capture manifest: if the definition changed, its task identifiers changed, and the smoke is stale. That gives `launch` a precise check to warn on — "no passing smoke for this manifest" — without needing to guess at what edits matter. Warn rather than refuse: re-launching after a fix, or resuming, are both legitimate reasons to skip it, and a hard gate would only teach people to bypass it. **And *cannot say* warns too**, which is the same argument read the other way: a journal that will not open, or one whose lines are damaged, leaves the question unanswered, and staying quiet there asserts *rehearsed* on no evidence at all. Damage counts for a sharper reason than symmetry — a torn line is what a crash mid-append leaves at the end of the file, which is exactly where the newest smoke is, so a fold that read past it would report the pass that smoke had superseded. **Damage past the last readable event, specifically**, which is the only damage that can hide a rehearsal: a fragment below one is older than a record that did read, so losing it can only under-report coverage, and warning on it would repeat `JOURNAL_DAMAGE` — the tend's item, and the one place it can be cleared — on every launch for the rest of the workspace's life.

**Three things are compared, because no one of them covers the others.** The identifiers are the half that can name a number — *3 of 12 tasks in this capture* sends somebody somewhere, where a bare mismatch does not — and they are compared as a subset, because adding a task to a rehearsed definition is the case worth naming and removing one is not. But `task_identifier` deliberately hashes execution limits and *not* the sample count, the epochs, or the selection, so that raising either leaves existing logs resumable: a dataset that doubled therefore keeps every identifier while being a materially different night, and a rehearsal blessing it would report a run nobody is about to make as rehearsed. The digest is what notices, and it is comparable at all only because the slice rides the workers — a smoke that captured under its own limit would differ every time. It is asked only where the identifier sets are *equal*, since a capture that dropped a task hashes differently for a reason the subset rule has already decided is fine. And the **scan configuration** is the third, because the digest hashes the tasks and the run's shaping and not `Manifest.scan`: a scanner added after a passing smoke reviews every transcript of the run having been exercised on none, and a `scan_model` changed since is a model whose context window the rehearsal established nothing about.

**The newest smoke is the answer, whatever it concluded.** Folding the journal back to the most recent *pass* would let a rehearsal that just failed sit behind a passing one from an hour ago, and report the launch as rehearsed on the strength of a run whose successor said otherwise — which is not a reading anybody would make from the same journal by hand. So the last `smoked` event wins and a last word that is not `passed` covers nothing.

Almost nothing about a smoke needs special machinery. It is a run with a sample limit, a time cap, and a different log directory, then recorded in the journal as part of the same story. Two things turned out to be exceptions: the cap, for the reason below, and being **tended** — which it is not.

**Built untended, and the reason is the cap's own stopping primitive.** `inspect ctl` cancellation finalizes a log with `status="error"`, which `observe` reads as a task worth retrying, so a capped rehearsal under a tend would respawn everything it had just stopped. A smoke therefore spawns once, watches the directory, stops on its deadline, folds its scan and reports — nothing reconciles it in between. That also leaves step 16's deferred *do not start this again* record deferred, since a rehearsal never enters the path that would need it.

**Its machine state is its own, and this was a real defect before it was a rule.** The in-flight record accounts for spent attempts per identifier and `reconcile` stops respawning a task after two of them — so a rehearsal writing into the run's record spends the run's attempt budget. Two smokes of a two-task definition left both tasks stalled before the real launch had run a single sample. A rehearsal's selection documents and in-flight record live under `.steward/smoke/` with its logs, and one `rmtree` is the whole of clearing it.

**Its upstream dependency landed whole.** Redirecting a definition's `log_dir` needs an override the definition cannot pre-empt, which no environment variable can supply — a variable is a *default*, and an explicit argument beats a default. `eval_set()` now has a default of its own (§2.1a), which does not change that: it widens which definitions omit the argument, and an omitted one still has to be redirected to Steward's answer rather than to the worker's cwd. Selection documents carry both halves in their `overrides` container (execution.md, items 4 and 8), so the log directory and the dataset slice both work for script definitions as well as Flow ones.

**A name collision nearly hid the slice: `max_samples` is concurrency, not a limit.** The override that landed beside `log_dir` bounds how many of a task's samples run *at once*; a smoke needs `eval_set()`'s separate `limit`, which bounds how many run *at all*. Both are in the container, for the reason the others were: `task_identifier` hashes a task's *execution* limits (message, token, turn, time, working, cost) and not its dataset slice, so truncating a worker's dataset cannot desynchronize it from the capture manifest.

**And the slice is taken *inside* whatever the run already selects, because the three selectors move as one.** `eval()` refuses `sample_id` beside `limit` or `sample_shuffle`, so every layer combining two sources of them takes all three from whichever source spoke — which makes a bare `limit=2` destructive rather than additive. Over a run selecting `(100, 200)` it clears the window and runs samples 0 and 1: a rehearsal of samples the run will never touch, recorded against the intended manifest's digest as successfully rehearsed. Every case is representable, so the rehearsal narrows rather than replaces — the front of a window, the first of a set of named ids, the shuffle kept and its front taken — and it reads what is *in force* rather than the override alone, since a definition calling `eval_set(limit=(100, 200))` records that in `options` with no override at all.

**`max_workers` is borrowed from the run and nothing else about the fleet is.** A rehearsal spawning one process per task is not the run when the operator has constrained the run to four, is a burst of startup cost they had already declined, and on a machine sized for four is a rehearsal that fails for a reason the launch would not have hit. `max_tasks` is deliberately not borrowed: it bounds how much runs at once, and a rehearsal is already bounded twice over by its slice and its cap.

**The slice rides the workers and not the capture**, which inverts what a launch does and is the decision the rest of this section rests on. A capture made under the limit would describe the *rehearsal*: per-task counts of two, and a `manifest_digest` differing from the real launch's for the one reason that does not matter. Capturing whole and truncating per worker means the manifest a smoke reads **is** the manifest the launch will read — so the validity check above can compare both the identifiers and the digest rather than guessing from one of them, and the population it reports is the run's.

**The digest is the artifact, and it had been named for three documents before it was defined.** agent.md §9's *trust the artifact, not the exit code* lists four, and this was the one with no mechanics anywhere. It is `.steward/smoke/digest.md`, printed to the terminal in short form and rendered whole to the file: the verdict first, then what ran and how big the run behind it is, then what the samples did, then the checks that produced the verdict, then what the scanners found, then every model's context window. The rule matters more here than at the other three gates, because under `fail_on_error=False` a rehearsal exits zero for most of what it is looking for.

**It also checks four things about how the run is configured, which is a widening of what a smoke is for.** The original list was failures that stop a run — a definition that will not import, a key that is wrong, a sandbox that will not start. These are failures that *do not* stop it: they produce an eval that completes, scores, and is quietly wrong.

- **The scanners run and their findings are reported.** The built-in rides every launch already, so a rehearsal exercises the whole scan path for free — and a scanner that throws on five thousand transcripts is exactly what a rehearsal is for. The smoke folds and **finalizes** its own scan, which no other non-signoff path does and which is simply true here: a rehearsal is terminal the moment it ends.
- **And the scanners *reached* every transcript**, which the findings cannot say. A scan that recorded nothing and a scan that found nothing produce the same empty list — no findings, no errors, nothing thrown — so a rehearsal whose scan path never wrote a row reported ready, having established the one thing it was meant to establish least. Coverage is folded the way the tend folds it, recorded rows against landed samples — rows rather than findings, since scout writes one for every transcript-scanner pair whatever the verdict, so a spotless review of four transcripts reads as four and a shortfall really is transcripts nobody answered for. **What blocks is nothing at all, not anything short of everything.** A definition's scan `filter` is a SQL clause applied per sample and an excluded transcript is never scanned — no row, no snapshot entry — while the filter reaches Steward only as a config hash, so a partial count cannot be told from a correct one and failing on it would fail every filtered definition on every rehearsal. A filter narrows *which* transcripts a scanner sees without stopping it running on the ones that match, so *no scanner answered for any transcript* is still the broken path; partial coverage states its number and passes. Waivable by name for the filter selective enough to match none of a two-sample slice.
- **Every model resolves to a context window.** Not *is it in the model database* — providers alias an unregistered frontier name onto a known entry before any lookup, so a pre-deployment codename gets a real window and is fine. What is checked is the number the runtime ends up with, because the alternative is `DEFAULT_CONTEXT_WINDOW`: 128000 assumed with one log line, and oversized tool-output shrinking disabled with none. The digest names the entry an aliased model landed on, since *you are getting the frontier's window* is the sentence worth reading. **Every** model: the models under evaluation, the scan model, and each task's `model_roles` — a grader silently running at an assumed 128000 is the same failure arriving where nobody thinks to look, and a definition whose interesting model *is* a role would otherwise have been checked only on its main one.
- **Reasoning is replayed to the model.** At two observation points, because they can disagree: `ModelEvent.input` is recorded after the history filter and says what Inspect decided to send, and `ModelEvent.call.request` is the body that was actually posted — the only thing that catches a provider converting a reasoning block it could not match back into plain text. The rehearsal asks for every raw call rather than the first five per model, which costs nothing on logs that are thrown away.

**And it reads what the samples did, which is not what the tasks did.** Workers force `continue_on_fail` on, which is what supervision depends on and also what makes *the task finished* worthless as a rehearsal's verdict: a wrong key, a sandbox image that will not start and a scorer that throws all land as errored samples inside a log whose status is `success`. Measured rather than argued — four samples landed, two errored, and the rehearsal reported ready. So the sample records are classed the way a tend classes them, through the same `class_summary` the decision queue uses, and any errored sample fails the rehearsal. An *operator-limited* sample is reported and does not, because it ran as designed.

**The other half of the same mistake is that `settled` and *finished well* are different questions.** The watch stops on a log that finalized `error` or `cancelled`, and it should — a task that died is not one to keep waiting for. But a task-level failure lands no errored samples to class, so the census sees a clean run of zero samples and the rehearsal reports ready again by the other route. Every task whose current log is not a success is named, classed on `task_error_class` the way a failed attempt is during the run.

**And a third reading of the same mistake: *finished* is not *ran what was asked*.** A log finalizing `success` while holding one of the two samples the slice named lost a sample somewhere that left no error behind, and every count downstream reads the smaller number as the whole truth. What the slice asks of a task is computable in one line — `min(samples, task.samples) × epochs`, and it collapses to that for every selection shape precisely because the capture is untruncated — so it is checked against the records that landed. Counted off the records rather than off the header, which are two different claims: a document saying *3 samples* in one line and *expected 4* in another is only useful if both came from the same count.

Unlike the three checks, this one is not waivable. `--accept` waives a question about configuration that a person can answer better than Steward can; a sample that errored is not a question, it is the answer arriving. Somebody who wants to launch anyway still can, because the gate on the real launch only ever warns.

**A failed check stops the launch; a check that could not run does not, and neither does one with nothing to check.** Three verdicts rather than two, and the distinction is load-bearing in both directions. *Unexercised* is a non-reasoning model, which would otherwise block every such eval on the day this shipped. *Undetermined* is a provider SDK absent from Steward's own interpreter, which is a fact about this machine rather than about the run. And a real failure can be waived by name, recorded in the journal, because a gate with no escape is a gate people route around.

**This is the one place Steward reads a whole eval log**, which agent.md §8 otherwise forbids outright. It is a named exception rather than a hole, and it holds because four bounds hold at once: only `.steward/smoke/` and never `logs/`, enforced by the path rather than by discipline; bounded by the rehearsal's own slice; paid once at launch rather than sixty times a night, which is what the context budget is actually about; and the transcript never leaves Steward — what travels is a verdict and a count, the same shape the anomaly census already has.
...
**Stopping is a `finally`, and it has a second half.** Untended is what makes a rehearsal cheap and also what makes a leaked worker permanent: nothing will ever reconcile one, so an exception on the third task of five, or a `Ctrl-C` at minute nine, leaves processes holding sandboxes and spending tokens against a smoke that has already printed its verdict. Every exit therefore goes through one door. And a cancel returns as soon as `inspect ctl` accepts it, while the worker is still finalizing its log and writing its scan rows — so the rehearsal waits, bounded, before reading the directory and folding the scan. Reading in the next breath under-reports what ran and prunes rows that land a second later.

**A drain that gives up is a failure, and the next rehearsal will not clear the directory under it.** The wait is bounded, so it can end with a worker still running — and that used to be a line in `steward.log` and nothing else: the tasks had settled, so the cap never fired, and the digest was free to report *ready* while one of this rehearsal's own processes was still generating against the account the run is about to use. Nothing tends a smoke, so nothing would ever stop it. Two changes, one for each half: the stragglers are named among what went wrong, which fails the rehearsal; and `prepare` refuses to `rmtree` a directory whose in-flight record still has live workers in it, because clearing it takes their logs and their record away from the only thing that could still be asked to stop them.

**The rehearsal records its scan under its own id, and a redirect is why that is not merely tidy.** `scan_dir_location` is `{scans or log_dir}/scan_id={scan_id}`, so a definition that sends its scan rows somewhere shared takes the log directory out of the answer entirely — and under the run's own eval set id the smoke then resolved to *the run's own scan directory*: rehearsal rows written into it, its summary rewritten, and, since the finalize prunes rows naming logs outside the directory it was handed, the run's rows pruned using the rehearsal's log directory. Redirecting the rehearsal's scans locally does not fix it, and that is the part worth recording: a worker reads `ScannerConfig.scans` out of the definition it executes, so it would go on writing to the redirect while Steward initialized somewhere else, and the worker-side check would refuse at startup for the directory it found uninitialized. A distinct id fixes it whole, because every path — init, the workers, the fold, the finalize — takes the id from the plan. What remains under a redirect is that the rehearsal's rows sit beside the run's rather than inside `.steward/smoke/`, which is the definition's own instruction about where scan rows go.

**And the rehearsal's workers get the run's retry budget**, through `worker_overrides` rather than the raw manifest ones. The real fleet is given `DEFAULT_RETRY_ON_ERROR` where neither the definition nor the run named a number; a rehearsal on the raw overrides ran at inspect's own default of none, so a transient blip errored a sample the launch would have retried — which now fails the rehearsal — and the retry path itself, which every worker of the real run depends on, went unexercised by the thing that exists to exercise it.

**A launch-shaping flag is refused rather than ignored.** `--no-timer`, `--sync`, `--log-root`, `--tend-interval`, `--samples-ramp` and the rest were all accepted beside `--smoke` and quietly dropped, because a rehearsal commits nothing, arms nothing and tends nothing — and the follow-up command printed after a passing smoke carried only the flags the rehearsal had *used*, so `steward launch --smoke --no-timer` printed a bare `steward launch` that arms one. Refusing is better than preserving in the printed command: a flag echoed back would say the rehearsal had run under it. It is the same refusal `--samples` already gets outside `--smoke`, pointing the other way.

**An empty capture is not a passing rehearsal, and it read as one for the same reason everything else in this section did.** `all(...)` over no tasks is true, so the watch returned at once, nothing spawned, every check read *unexercised*, no error existed — and the journal recorded a pass at `tasks = landed = population = 0`. A definition edited to nothing was blessed as rehearsed and ready by the machinery that exists to catch exactly that.

**And the counts are over the current attempt, never the union of attempts.** A rehearsal spawns once and respawns nothing, but `eval_set()` retries a failed task *inside* the worker, so one task can land two logs — and a completeness check summed across them let two one-record attempts satisfy a two-record slice while the log that is actually the result held one. Everything else in Steward counts the current attempt and asks about history separately (`failures` still classes a superseded attempt's errors, because an error that happened, happened), so the slice check, the landed count and the coverage numerator all moved onto it — the last by passing `reused_samples` rather than asserting a rehearsal never resumes.

**And a rehearsal nobody can read the result of has not passed.** The digest write and the journal append were both swallowed into `steward.log`, which is the wrong reading of *best effort* for these two: the digest is one of agent.md §9's four artifacts, and the journal is what the next launch reads to decide the run is rehearsed. Lose the first and keep the second and the gate says *rehearsed* while the terminal points at a file that is not there. A write that fails is now an error of the rehearsal, the verdict is recomputed over it, and the journal records **that** — which also retires any older pass, since the newest smoke is the answer whatever it concluded.

**And the correction is appended rather than merely returned.** An append is a write followed by an `fsync`, so a failure does not mean the line is absent: an `fsync` that failed after the write landed leaves a journal whose newest smoke says *passed* while the value handed back says otherwise, and the fold takes the newest whatever it concluded. So a failed append is followed by a second one carrying the failing verdict, which settles both shapes in the safe direction — where the first line landed this supersedes it, and where it did not this is the only record.

**The rehearsal captures under what the launch will capture under, and settles the fleet's environment before it spawns.** Both are the same failure in two places: a gate that blesses a configuration it never exercised is worse than no gate, because it is read as evidence. A workspace launched once with `-A` and `--epochs` carries them in its committed manifest and a later bare `launch` reuses them, so a rehearsal capturing the definition's own defaults establishes that a run nobody is about to make works. And the scan model and the notification channel reach workers through this process's environment, so settling them after the fleet has started leaves it scanning with the shell's answer and unreachable, while the digest reports the workspace's.
...
The entire value proposition is "don't bother me unless it matters," and both failure modes are bad in the same way: notify too much and the human stops reading; notify too little and they discover in the morning that $400 went somewhere wrong. So notification policy is *the* tuning knob for how much autonomy the human has actually granted, and it belongs in `_steward.yaml` — as a setting rather than in `policies`, because a mechanical notification has to fire when no agent is present to interpret a sentence.

**The channel is in that file too, which reverses the position this section originally took.** The old rule was that the channel stays out and is named only by the environment, inherited from Inspect's reference-only discipline. What changed the judgement is the scheduler: a tend firing at 02:00 inherits almost no environment, and the file key is the one spelling still there when it does. So `notification` is a Steward setting in all three spellings — the key, `STEWARD_NOTIFICATION`, and `steward launch --notification` — and it is the second carve-out from the overrides table after `log_dir` (*A config file may not say anything the definition can*). What is kept from the discipline being departed from is that the value never *travels*: nothing prints it, and the copy propagated into the log directory has the key replaced by a note saying so. A URL in a committed file is a real cost, and the template recommends `.env` for anybody who would rather not pay it.

**One channel, two consumers.** The setting reaches every worker as well: exported as `INSPECT_EVAL_NOTIFICATION`, *and* applied as the `notification` override in each worker's selection document — because a worker's `eval_set()` only builds an Apprise instance when its own argument is truthy, so the variable alone leaves the fleet silent while Steward posts normally. Naming the channel Inspect's way works too, and Steward posts to it: the relationship is reflexive in both directions, which is what keeps the two from ever being two destinations.
...
### 11.1 Six kinds, and four of them are not the agent's to send

Every post carries a **kind**. It is not decoration: it sets how the post presents and who may send it, and its whole job is keeping the one message that needs a person distinguishable from the ones that merely inform.

| kind | means | sent by |
|---|---|---|
| `attention` | something worth knowing, and work continues — a class growing fast, a task that died, a scan finding, a re-run proposed | the agent, and Steward when an item appears at that level |
...
| `progress` | tasks finished this turn | **Steward only** |
| `clear` | the decision queue emptied and work continues | **Steward only** |
| `gate` | tasks finished, scans drained, anomalies settled; the run is waiting on `signoff` | **Steward only**, latched |
| `signed_off` | the run was accepted | **Steward only**, terminal |

The split is the point. `attention` and `stopped` carry judgement, which is why they are the agent's. The other four are either latched, terminal, or read off state the agent does not own, and `steward notify` refuses them by name.

**Kind and verdict are two axes, and conflating them is what made `complete` mean two things.** A kind is the post's *reason*; the verdict is the *run's state*, computed every turn, and it rides in the title of every post regardless. A `progress` post fires while the verdict is ✅, which is precisely the case a one-axis vocabulary cannot express. This is also why the terminal kind is called `signed_off` rather than `complete`: `Verdict.COMPLETE` (🏁) already means *finished and unaccepted*, which is the state `gate` announces.

**One post per turn, at most, whatever changed.** A turn is one moment and a reader wants one message about it, so the kinds have a precedence — gate, then stopped, then attention, then clear, then progress — and the body names every reason. A turn that finishes three tasks and empties the queue is one message, not two; and because the tend is already the clock, a sweep that finishes five tasks in one interval posts once naming five. A long night tops out at one post per interval.

**Every trigger is a set diff between turns, never a count.** One item resolving while another arrives is not *no change*, and a task finishing while another is reset is not *no progress*. The diffs are the ones the turn already computes — the items that appeared, the items that closed, and the tasks complete now that were not before.

**Which is also what implements the latches, with no new state.** `gate` posts once because `signoff_ready` appears in exactly one turn's diff, and its id is keyed on the manifest digest — so a later `launch` that changes the task set mints a new id and the next settling posts again. That is the case a manual convention would get wrong, since a relaunch is exactly where nobody would think to re-arm anything. The parked-worker `stopped` is the same mechanism: reconcile sees a park in the control-channel read it already performs ([execution.md](execution.md), *The parked worker*), it becomes an item, and the item appearing is the post. A worker that waits six hours produces one post, not thirty-six.

**A channel reaches a person, so it carries the person's items and not the agent's.** An item routed to the agent — an `unreadable` log, a failed action — is routed there precisely because a person is not the one who should look at it, and a post naming it is the notification advertising work its reader was told not to do. The projections a human opens on purpose (`status.md`, the terminal) still show both, grouped by owner; a channel is the one surface where the reader did not choose the moment, so it is the one surface that filters. The title follows: it counts what this reader has to act on and says *N decisions need attention*, where `status.md` splits *needs a person* from *for the agent* because one of its readers **is** the agent, for whom the split is the routing. *Decisions* rather than *tasks*, since `drift`, `degraded`, `unsupervised`, `timer_drift` and `signoff_ready` are facts about the run with no task behind them — and it is already the word the body uses where it runs out of room.

**Named by its workspace, because one channel serves however many runs a person has going.** Every title here is a sentence about *a* run, and *2 decisions need attention* names none; the directory name is what a person calls it in conversation and in `cd`.

**A post carries a bounded amount of everything, and the last bound is the width of a line.** The list is capped at eight with the remainder counted, the table at twelve rows with the totals kept, and an item's own summary at a length somebody reads on a phone. That last one is set above every summary Steward composes, so what it ever trims is text Steward did not write: `unreadable`, `degraded` and `action_failed` each embed somebody else's exception, which arrives with an absolute path and no length in principle. An action is never trimmed — a command is either paste-able or worthless. And a `gate` post names no items at all, because 🏁 is returned exactly when every open item is `signoff_ready`, so the verdict line already *is* that item's sentence. Tasks are named as shortly as the table beneath names them, which reverses `items.py`'s *a summary names its task in full* for the one surface where the table is on screen: a model every row shares is said once in the totals rather than after every name.

**Unless nobody is picking them up.** An agent's item is only the agent's while there is an agent, and the thing a person has to do about a workspace nobody has attached to is attach to one. So the agent's items reach the channel when no collection has happened in the last two tends — counted in tends rather than minutes, because the question is whether an agent has had the *chance*, and the chances are turns. Two, since the first is the turn the item appeared on. Never having collected needs no horizon at all: it is not an agent that went quiet, it is a workspace no agent was ever attached to. Nothing new is recorded for this — `collect` already stamps the journal, and the collection age already sits beside the tend age precisely so that *the timer stopped* and *nobody is looking* are distinguishable ([agent.md](agent.md) §2.2).

**No clock trigger.** A periodic digest would need a period, and inventing a cadence key beside the tend interval would be a second scheduler. `progress` covers most of what a digest was for.

**An edge that reached nobody is not spent.** The observation recording *these items are open now* is written before anything is sent, and it is what stops a condition being reported every ten minutes — so a notifier unreachable for the one minute the gate landed in would consume the gate permanently, and every turn after it would see a run in which nothing changed. A turn that could not deliver therefore writes down what it was carrying (`undelivered`), and the next turn subtracts that from the baseline it diffs against. The edge is retained rather than the message retried: the following turn recomputes a post from the state it finds and includes what was owed, which is right for a message describing a moment rather than a queue of them. A channel that will not build counts the same way, since the reader was not told either way.

**The one thing with no edge to stand on is a turn that raises.** A malformed `_steward.yaml`, an unreadable log directory, an expired credential: each fails identically every interval, never reaches its own observation, and leaves `status.md` frozen at whatever the last good turn said. So that post is written down (`notified` in the journal) and latched per distinct reason, released by the next turn that actually runs — one message per failure rather than forty before morning.

**Post freely.** With no channel configured, nothing is built and nothing is sent — so the agent never has to check whether notification is set up, and a machine with no target does not accumulate failures. The cost of an unnecessary `attention` is one line in a channel; the cost of a skipped `stopped` is a run that waits all night for an answer nobody knew was wanted. What a run with no channel does get is **one line at `launch`**, because this is the feature most costly to miss and its absence is silent by construction — suppressed by `notification: false`, since nagging somebody who declined is the overdone version of the same idea.

### 11.1.1 Dialect, detected rather than assumed

**Slack's `mrkdwn` is a different dialect, not a subset of markdown.** No headings, so `## the run` arrives as two characters. No tables, so a pipe table arrives as pipes. `*bold*` rather than `**bold**`, and `<url|text>` rather than `[text](url)`. `status.md` is built from exactly those four constructs, which is why the file renderer is not reused for a post — what is shared instead is everything below markup, so a post and a `status.md` cannot disagree about what a turn found, only about how it is spelled.

Steward cannot learn the channel from configuration in the general case, since the operator may have named it Inspect's way. So the dialect comes off the **built Apprise instance**: each plugin declares a `notify_format`, and that declaration is the answer, with the Slack family as the one named override. Reading a plugin's declared output format is not reading a credential.

Targets that disagree are **partitioned and sent once per dialect** rather than reduced to the worst of them. Every dialect is rendered directly and Apprise converts nothing — measured rather than assumed: its markdown-to-HTML renders a fenced block as `<code>` inside `<p>`, which collapses the progress table into one run-on line, while text-to-HTML preserves the alignment. So the HTML dialect is the text rendering escaped inside `<pre>`.

### 11.2 What Inspect already provides

`inspect_ai.util.notify(message, title=None)` — async, best-effort, backed by [Apprise](https://appriseit.com), so every channel (Slack, email, SMS, desktop, webhook) comes from Apprise's URL DSL rather than from Inspect. It never raises and never blocks more than five seconds, which suits a caller that must not stall a tend. Apprise is optional upstream and a **proper dependency** of Steward, so there is no import guard here and no configuration that resolves to a channel Steward cannot reach.

Three properties of it shape Steward's design:

**Configuration is reference-only upstream, and Steward departs from that deliberately.** Inspect refuses notification URLs as API or CLI arguments — you pass `True` (read `INSPECT_EVAL_NOTIFICATION`) or a path to an Apprise config file — to keep credentials out of source, shell history, process listings, and eval logs. Steward's `notification` setting accepts a URL in every spelling, and §11 above gives the reason: the file key is the only one a 02:00 tend still has. What is kept is the half that matters most here — the value never travels. `status` renders policies alone, nothing prints it, and the `_steward.yaml` copy propagated into the log directory has the key replaced by a note naming why. `build_apprise` is still what parses it, so no second copy of the URL handling exists to disagree with upstream's.
...
`build_apprise()` and `init_apprise()` already exist and do precisely what is needed, but live in the private `inspect_ai.util._notify`. An earlier draft of this section asked for them to be made public. **That ask is withdrawn: private is correct.** Importing them is the same reach-around Steward already makes for `inspect_ai._eval.evalset`, `_control.discovery`, and `_cli.main`, and what is wanted is the credential discipline `build_apprise` enforces, not a second copy of the URL parsing — which one import buys without asking upstream to widen a surface for one caller.

`notify()` itself is not used at all, and the reason is the dialect rather than the scope: it passes no `body_format`, so every target is handed the raw body whatever it declared. Steward calls `instance.notify(...)` directly, which is about ten lines — `anyio.move_on_after(5.0)` around a worker thread, every failure swallowed into `steward.log`. Two properties matter to a caller that must not stall a turn, and both are preserved: bounded, and best-effort. **Apprise catches a plugin's own exception and answers `False`**, so the return value is what says a channel is broken; a caller that only guarded against a raise would record every dead channel as delivered.
...
> **Neither mitigation is built, and the blind spot is therefore open.** Both need the distribution machinery step 30 declined. The second also wanted a cross-run store, and step 33 built one — but it indexes *logs by task identifier*, which answers *has this exact task run before* and not *what did this scanner report last month*; the comparison needs a record of scan rates across runs that nothing keeps. What stands in for them is one sentence in the agent's brief — *197 of 200 flagged is the alarming case, not the noisy one* — which is a prompt where the design asked for a mechanism, and should be read as owing the mechanism rather than as having replaced it.
>
...
- **Accepting known holes must be explicit, not blocked.** Real evals ship with failures nobody intends to fix. Refusing to sign until everything is clean would just push people to fake resolutions, so signoff records accepted exceptions by name — "2 samples accepted as errored" is a signed statement, not a silent gap. **The gate's refusal is therefore a routing instruction rather than a quality bar**: what it refuses is not a hole but an *unnamed* one, so every blocker names the act that answers it and `accept` and `dismiss` are among the answers offered.
- **The gate returns every blocker at once**, coarsest first — tasks not settled, a live worker on an orphan, an open window (including `limit:`, where the readiness item deliberately looks away), errored samples no ruling covers, and a signature that already stands. A person who fixes one refusal and meets another has walked exactly the loop the gate exists to collapse.
- **It is an attestation, not access control.** What is the human's is the *decision*, never the keystroke: an agent that notices a run is ready, tells them, and carries out their answer is doing its job, and it records their name exactly as `rule --by` does for a ruling it is relaying. So the design records the signer rather than gating the caller, and a signature nobody asked for is visible rather than prevented — the same bargain a commit author line makes.
- **It can be invalidated.** A scan landing after signoff, or a later invalidation, re-opens anomalies. The signature stays in the journal as a record of what was believed at the time, and a fresh one is required. **The test is temporal rather than "nothing is open now"**, and the difference decides a real case: a window that opened at 3am and was ruled at 4am is closed by the time anybody looks, and letting the old signature come back into force over a finding its signer never heard of is the certification-by-default this section opens by refusing. So a later finding un-signs permanently, even once ruled, and the remedy is the honest one — signing again.
- **It ends the run.** Signing disarms the timer, because a signed run that goes on tending every ten minutes is spending money against an explicit instruction. That is also why the *terminal* notification is sent by the verb rather than by a turn: there is no next turn to send it.
- **There has to be something to attest to, which the gate did not ask.** Every refusal above is about some part of the results being unsettled, and nothing is unsettled about nothing: an empty manifest reports zero missing and zero incomplete, so a capture that enumerated no tasks — a definition edited away, an argument that filtered every task out — passed the whole gate and received a signature over an empty set. A task whose dataset is empty does the same one level down, once its `0/0` log lands `success`. This is the one refusal with no remedy naming a decision, because there is no naming act for it: a signature over no results is not a caveated attestation, it is a statement about nothing.
- **And the signature says how much of the run the scanners actually read.** *No accepted exceptions* over a census that covered none of it is the same failure as a silent hole, arriving where nobody had put a predicate — a completed four-sample run with zero transcripts scanned recorded `exceptions=()`. It is **not** refused, and the distinction is the one this section already draws: a coverage gap has no closing verb (scout's resume over the scan directory is not something Steward has), and several perfectly correct configurations produce one — a scanner added at a re-launch, a `ScanSpec` that filters what its scanners see. A blocker answerable only by a flag every signoff carries is the same as no gate. So the number is recorded by name among the exceptions, read off the terminal **finalize** rather than off the turn: curation has just moved the superseded attempts, the finalize drops the rows naming them, and the turn's own coverage can therefore be wider than the census that survived.
...
**Only the indexing half waits to be asked, though.** Removing the row is what keeps the store and the directory in agreement, so it happens at every signoff that archives something a store holds — with or without the flag. Gating the two together, as the first implementation did, meant a project that stopped publishing went on serving results it had superseded, which is the disagreement this paragraph exists to rule out. A removal the store refuses is journalled and retried at the next signoff, since curation has by then put the log beyond anything else's reach.
...
- **Steward already computes it, and the workaround was already paid for.** `latest_completed_task_eval_logs` encodes exactly this predicate and is private and exported nowhere, which looked like a reimplementation curation would owe. It does not: `ObservedLogs.current` answers *which attempt is the result* and `_current` also asks whether an attempt answers the run's shape, both built long before this. Curation consumes `TaskObservation.superseded` and adds no third predicate. **The rule it applies is *not current*, never *the status looks bad*** — a task latched short by an acceptance has a current log whose status is `error`, and that log is the result the attestation covers with a caveat, so choosing on status would archive the evidence for the exception the signature just recorded.
- **Reconcile already archives every orphan's logs every tend, and what is left for signoff is exactly its complement.** The two partition on manifest membership, so they cannot fight over a file even in principle: an args edit produces a *new* identifier, so the old one is an orphan and was archived long ago, while what waits for signoff is multiple attempts of a still-live identifier. Curation refuses to run at all while a worker is still on an orphan, which is the one condition that says the directory is not still — a refusal none of this section's earlier drafts named.
- **A move that fails is reported and does not fail the signature.** The signature is the person's act, and a filesystem that would not cooperate must not unmake a decision they already made; the next signoff tries again. A failed *disarm* is the opposite call and raises, because a failed move leaves a tidy-up undone where a failed disarm leaves a run spending money against an explicit instruction.
...
Two edges follow from the same place. A signed log later superseded by an amendment leaves a store row pointing at an archived path, so archiving removes the row — which the same command does, in the same pass as the curation that archived it. And a task signed *with* accepted exceptions carries a caveat that lives in this project's `anomalies.md` and travels nowhere: whoever reuses it gets the log without the footnote. That was the sharpest open question about what should be publishable at all, and it is answered permissively — **a signature is a signature**, and withholding a result a person explicitly accepted would make the store lie in the other direction about what a project produced. The hole is taken knowingly and its cost is stated to the person deciding rather than engineered around.

**And it is asked for, never configured.** Publication is the one act at the end of a run that nothing does by default and no `_steward.yaml` key can turn on: a store is frequently shared, so publishing exports this team's results into a cache other people read, and a setting that could say *always* is publication nobody was asked about. So `--publish` is the whole surface. What makes that workable rather than merely safe is that the ask is built: the readiness item names the configured store and tells the agent to put the question, and a signoff that published nothing while a store sat configured says so on the way out — the last moment anybody is looking, since the timer comes down immediately after and a signed run never tends again.

**It pins to a manifest digest, not to a run.** Because a project's definition evolves (see *A project, not a run*), an attestation has to name what it covered, and the manifest digest is the natural handle — *derived* rather than minted, so nothing has to allocate or persist an id for it. It also gives invalidation a second precise trigger alongside the anomaly one: the definition changed, so what was signed is no longer what is current.

**And the digest test is strict, which settles a sentence this section used to get wrong.** An earlier draft had a purely additive launch leaving an existing signoff standing over the tasks it covered while the new tasks were plainly unsigned — a signature covering *part* of a manifest. That is not something a digest can express and not something a person signed: what they attested to was a set of results, and the set is now different. So any launch that changes the task set un-signs the run, additive or not, and the remedy is one command. A launch that archives supersedes part of what was signed, which is a further reason that case escalates rather than proceeding quietly.
...
**Raised at adjudication, not inline.** An operator kill is the one anomaly whose author already knows about it — asking the person who killed a sample to rule on the kill while they are still triaging is the system nagging them about their own act. So the window is opened and journalled the moment it is observed (the record must exist, and the data's residue must reach signoff), but it produces no inline item and does not hold back the signoff-ready invitation: it waits in the fold and the status's anomalies block for the adjudication conversation, where it must be ruled like everything else. §15's rule is doing the work here — the run keeps going and anomalies are settled afterwards, and this class is the purest case of *afterwards*.
...
4. **Who commits the journal?** *Resolved — it stays the human's job, and `signoff` owes something else instead.* `init` prepares the repository and nothing commits, which looked like a gap for the durability-through-git story to fall through. It is not one, because a commit is a change to the *user's own working tree*: a verb that made one would be Steward reaching past the four files it writes into a repository it does not own, and the tree it reached into would usually hold uncommitted work of somebody else's. What signoff owes instead is the property that makes committing safe at any later moment — **at the instant it returns, the record is complete and quiescent**: the timer is down, nothing further will be appended without somebody asking for it, and a commit taken tonight or next week captures the same thing. So the terminal act tells the person the journal is theirs to commit rather than committing it, which is the one line of the echo that names something the verb deliberately did not do.
