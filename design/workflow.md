...
**Two of these are written today** ([plan.md](plan.md) steps 12–13): `initialized`, and — from every executed tend — `observation`, carrying task-state counts, running/spawning/queued/stalled/orphan counts, whether the definition has drifted from the manifest, and the concurrency settings in force. That last field is not reporting material: it is what a turn reads back when `_steward.md` cannot be parsed, which is how degrading to the last known good is possible at all. `action` is written for an archive, with the log's origin, its destination, and its reason. The rest arrive with the steps that produce them.
