...
14. **Say in the log which metric is the headline.** `EvalResults.scores` is a list of `EvalScore`, each carrying a `metrics` dict, and nothing marks one as primary. So any reader rendering a task × model table — the natural shape of an eval set's result, and what a human actually wants from a status view — has to invent a convention, and every reader invents a different one. Two forms, and the cheap one is worth having on its own: a computed `headline` on `EvalResults` centralizing whatever ordering inspect's own view already applies, so that readers at least agree; and, richer, a way for a task or a metric to *declare* it, which is the half nothing can derive because it is the author's judgement.
