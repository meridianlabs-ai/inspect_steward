...
**The rehearsal consults rung 2 as well, read-only.** `launch --smoke` asks the store the launch's own question, through the same predicate (`_store.match`), and leaves out what it answers: a task the launch will copy in is not work the run does, and a workspace whose every task the store holds rehearses nothing and passes with *nothing to rehearse*. The `smoked` record still names every task the capture enumerated, so the launch gate's subset rule holds, and carries the skipped identifiers beside them so a later launch whose store no longer answers one can say that it will run unrehearsed.
...
**Both now have one, and Steward uses it (`_marks`).** Upstream's landed-log score edit — `edit_score` applying a `ScoreEdit` with history and provenance, `Score.unscored` as the NaN every metric and reducer skips, `recompute_metrics` rebuilding `results` — closed the gap this paragraph recorded. An `exclude` is written into the log by a detached runner: every score of every ruled sample becomes unscored with the reason, and the metrics are recomputed over the rest. A `zero` is not a value Steward can write, because what a zero is depends on the task's scorer; the runner obtains it by starting the task on just the ruled sample ids in a scratch directory under `.steward/marks/`, cancel-scoring each sample as it begins, and copying the scorer's verdict on that empty attempt into the main log. Applied is a state: the signoff gate refuses while a mark is unwritten, and the report says so.
