...
**It closes nothing.** `status` still shows a raised item, in full, because an operator still owes an answer; the verdict still counts it. The only projection that changes is the agent's own, where it is set aside and *counted* rather than hidden — an agent can read a label but cannot reason about what it was never shown.

**Which is exactly why only an operator-owned item can be raised.** Taking something out of the agent's queue without closing it is safe only where somebody else is going to close it. An agent-owned item raised this way is stranded: open forever, gone from the queue that would have brought it back, and owned by nobody who is looking at it. The agent's verb for its own work is `ack --by agent`, after it has actually resolved the thing, and the two are not interchangeable — one records a hand-off and the other records a disposal.
...
Named `raise_item` because `raise` is a keyword. Everything an operator types says `raise`.
"""
...
def raise_command(item: str, note: str, output_json: bool) -> None:
    ...
    # **owned by an operator, and that is the whole gate.** Raising takes an item out
    ...
    # Keeping it stopped mattering once a kind was both operator-owned and
    ...
    if output_json:
        ...
        if before is not None:
            ...
            click.echo(
                "  still open — an operator decides it; `steward collect` will not"
            )
...
def _nothing_matched(journal: Path, item: str, result: TendResult) -> str:
    ...
    Three rather than four now that the gate is ownership alone: every operator-owned item is raisable, so *matched but not offered* can only mean the agent's own.
    """
    ...
    if agent_owned := [entry for entry in named if entry.owner is Owner.AGENT]:
        ...
        return (
            f"'{item}' is the agent's own to resolve rather than an operator's, so "
