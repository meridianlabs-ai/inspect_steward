...
The one hazard the mechanism cannot defend against is the file being *unlinked* while a claim is held: the lock is attached to the inode, so the holder keeps it, the next acquire creates a fresh file and locks that, and two Stewards run at once. Nothing on the new file's side can see it, because the old inode is no longer reachable by name. `.steward/` is documented as disposable, so this is a real thing an operator can do, and the answer is the same one the starting-worker case takes — delete it when nothing is running (workflow.md, *Three categories, and the one that matters*).

**What is left is a wedged holder**: alive, holding the lock, long past what a tend should take. `flock` cannot be taken from one without killing it, so that is what happens, by default and unattended. Killing a tend destroys no work — workers are detached and outlive it, and every write a tend makes is already built to be interrupted, because an interrupted tend is reconciled by the next one either way. The alternative, refusing and leaving an operator to break it, is worth nothing at 2am, when there is no human and the entire point is that the run keeps converging.
